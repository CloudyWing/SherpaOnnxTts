...
public static class TtsEndpoints {
    ...
    private static void MapTtsGet(this IEndpointRouteBuilder endpoints) {
        endpoints.MapGet(
            TtsGetEndpoint,
            (string text, string? model, string? voice, int? sid, float? speed, string? format, ITtsService service) => {
                ...
                // Resolve Speaker ID
                // 1. Try 'sid' (integer)
                // 2. Try 'voice' (integer or string name)
                int speakerId = sid ?? 0;
                if (sid is null && !string.IsNullOrWhiteSpace(voice)) {
                    // Try parsing as integer
                    if (int.TryParse(voice, out int parsedSid)) {
                        speakerId = parsedSid;
                    }
                    // Try looking up by Name (case-insensitive)
                    else if (KokoroVoiceMap.TryGetValue(voice.ToLowerInvariant(), out int mappedId)) {
                        speakerId = mappedId;
                    }
                }
                ...
                TtsGenerationParameters parameters = new() {
                    Text = text,
                    Voice = model, // 'Voice' property in parameters is the Model Name
                    SpeakerId = speakerId,
                    Speed = speed ?? 1.0F,
                    Format = outputFormat
                };
        ...
        .WithDescription("Generates speech audio file (WAV/PCM) for download. Params: model=ModelName, voice/sid=SpeakerID")
